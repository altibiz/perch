...
let
    ...
    if path == [ ] then
      ...
      if !(attrs ? ${key}) then
        ...
        if tail == [ ] then
          ...
          if builtins.isAttrs attrs.${key} then
            let sub = keepAttrByPath tail attrs.${key}; in
            if sub == { } then
              { }
            else
              { ${key} = sub; }
  ...
  recursiveMerge =
    ...
    let
      ...
      mergeKey = k:
        if (a ? ${k}) && (b ? ${k}) then
          if builtins.isAttrs a.${k} && builtins.isAttrs b.${k} then
  ...
  keepAttrsByPath =
    paths: attrs:
    builtins.foldl'
      (acc: path: recursiveMerge acc (keepAttrByPath path attrs))
      { }
      paths;
in
{
